...
exports.uploadChildDocument = async (req, res) => {
    const { id } = req.params;
    const { url, public_id, name } = req.body;

    try {
        // Controleer of de benodigde velden aanwezig zijn
        if (!url || !public_id) {
            return res.status(400).json({ message: 'Missing required fields: url or public_id' });
        }

        // Documentgegevens opslaan in de database
        const updatedChild = await Child.findByIdAndUpdate(
            id,
            {
                $push: {
                    document: {
                        name: name || 'document', // Zorg ervoor dat je een naam hebt
                        url,
                        public_id,
                    },
                },
